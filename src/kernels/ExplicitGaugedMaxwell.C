...
#include "ExplicitGaugedMaxwell.h"
#include "Function.h"
#include "Assembly.h"
...
ExplicitGaugedMaxwell::ExplicitGaugedMaxwell(const InputParameters & parameters)
...
Real
ExplicitGaugedMaxwell::computeQpResidual()
{
  return _rho[_qp] *
             (_curl_u[_qp] * _curl_test[_i][_qp] + _grad_u[_qp].tr() * _grad_test[_i][_qp].tr()) +
         _test[_i][_qp] * _mu[_qp] * (_u_dot[_qp] + _grad_v_dot[_qp]);
}

Real
ExplicitGaugedMaxwell::computeQpJacobian()
{
  return _rho[_qp] * (_curl_phi[_j][_qp] * _curl_test[_i][_qp] +
         ...
         (_n[_qp] - 1) * _rho[_qp] * (1 / (1e-99 + pow((_curl_u[_qp].norm()), 2))) *
             (1e-99 +
              (_curl_phi[_j][_qp] * _curl_u[_qp]) * (_curl_u[_qp] * _curl_test[_i][_qp] +
...
//     ((_grad_u[_qp].tr() * (_grad_test[_i][_qp].tr())))
Real
ExplicitGaugedMaxwell::computeQpOffDiagJacobian(unsigned jvar)
{
  if (jvar == _v_id)
    return _test[_i][_qp] * _mu[_qp] * _dv_dot_dv[_qp] * _standard_grad_phi[_j][_qp];
  else
    return 0.;
}