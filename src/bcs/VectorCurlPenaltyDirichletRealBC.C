...
#include "VectorCurlPenaltyDirichletRealBC.h"
#include "Function.h"

registerMooseObject("ApolloApp", VectorCurlPenaltyDirichletRealBC);

InputParameters
VectorCurlPenaltyDirichletRealBC::validParams()
{
...
VectorCurlPenaltyDirichletRealBC::VectorCurlPenaltyDirichletRealBC(const InputParameters & parameters)
...
Real
VectorCurlPenaltyDirichletRealBC::computeQpResidual()
{
...
Real
VectorCurlPenaltyDirichletRealBC::computeQpJacobian()
{
  return _penalty * (_phi[_j][_qp]).cross(_normals[_qp]) * (_test[_i][_qp]).cross(_normals[_qp]);
}

Real
VectorCurlPenaltyDirichletRealBC::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _v_id)
     return _penalty * (_phi[_j][_qp]).cross(_normals[_qp]) * (_test[_i][_qp]).cross(_normals[_qp]);
  return 0.0;
