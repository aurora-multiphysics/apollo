...
#include "VectorTangentialPenaltyDirichletImagBC.h"
#include "Function.h"

registerMooseObject("ApolloApp", VectorTangentialPenaltyDirichletImagBC);

InputParameters
VectorTangentialPenaltyDirichletImagBC::validParams()
{
...
Real
VectorTangentialPenaltyDirichletImagBC::computeQpResidual()
{
...
Real
VectorTangentialPenaltyDirichletImagBC::computeQpJacobian()
{
...
Real
VectorTangentialPenaltyDirichletImagBC::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _v_id)
     return -_penalty * (_phi[_j][_qp]).cross(_normals[_qp]) * (_test[_i][_qp]).cross(_normals[_qp]);
  return 0.0;
