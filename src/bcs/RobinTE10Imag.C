...
#include "RobinTE10Imag.h"
#include "Function.h"

registerMooseObject("HammerheadApp", RobinTE10Imag);

InputParameters
RobinTE10Imag::validParams()
{
...
RobinTE10Imag::RobinTE10Imag(const InputParameters & parameters)
...
Real
RobinTE10Imag::computeQpResidual()
{
  ...
  RealVectorValue Ncu_im = _gamma_im->value(_t, _q_point[_qp])* (_u[_qp] - u_exact_re).cross(_normals[_qp]);
  return  (1/mu0) * Ncu_im * ((_test[_i][_qp]).cross(_normals[_qp]));
}

Real
RobinTE10Imag::computeQpJacobian()
{
  Real mu0 = 1.25663706e-6;
