    ...
    int dim_num, int* start_of_block, std::map<int,int>& libmeshToMFEMNode)
{
  ...
  for (int iss = 0; iss < (int)num_side_sets; iss++) 
  {
    for (int i = 0; i < (int)num_side_in_ss[iss]; i++) 
    {
      ...
      {
        renumberedVertID[j] = cubitToMFEMVertMap[1 + ss_node_id[iss][i * num_face_nodes + j]];
  ...
  if (order == 2)
  {
    ...
    mfem::FiniteElementSpace      *fes = new mfem::FiniteElementSpace(this, fec, Dim, mfem::Ordering::byVDIM);
